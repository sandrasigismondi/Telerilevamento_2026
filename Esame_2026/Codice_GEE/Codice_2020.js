...
var collection = ee.ImageCollection('COPERNICUS/S2_SR_HARMONIZED')
                   .filterBounds(aoi)
                   .filterDate('2020-09-01', '2020-09-30')              // Filter by date                                   // Filter by AOI
...
Export.image.toDrive({
  ...
  description: 'columbia_september2020',
  folder: 'GEE_exports',                        // Folder in Google Drive
  fileNamePrefix: 'columbia_september2020',
  region: aoi,
...
var collection = ee.ImageCollection('COPERNICUS/S2_SR_HARMONIZED')
                   .filterBounds(aoi)
                   .filterDate('2021-09-01', '2021-09-30')              // Filter by date                                   // Filter by AOI
                   .filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE', 20)) // Only images with <20% cloud cover
                   .map(maskS2clouds);                                  // Apply cloud masking
...
Export.image.toDrive({
  image: composite.select(['B2', 'B3', 'B4', 'B8', 'B11', 'B12']),  // Select RGB bands
  description: 'columbia_september2021',
  folder: 'GEE_exports',                        // Folder in Google Drive
  fileNamePrefix: 'columbia_september2021',
  region: aoi,
  scale: 20,                                    // Sentinel-2 resolution
  crs: 'EPSG:4326',
  maxPixels: 1e13
...
var collection = ee.ImageCollection('COPERNICUS/S2_SR_HARMONIZED')
                   .filterBounds(aoi)
                   .filterDate('2023-09-01', '2023-09-30')              // Filter by date                                   // Filter by AOI
                   .filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE', 20)) // Only images with <20% cloud cover
                   .map(maskS2clouds);                                  // Apply cloud masking
...
Export.image.toDrive({
  image: composite.select(['B2', 'B3', 'B4', 'B8', 'B11', 'B12']),  // Select RGB bands
  description: 'columbia_september2023',
  folder: 'GEE_exports',                        // Folder in Google Drive
  fileNamePrefix: 'columbia_september2023',
  region: aoi,
  scale: 20,                                    // Sentinel-2 resolution
  crs: 'EPSG:4326',
  maxPixels: 1e13
